...
2. The packet from the `netshoot` pod hops to the Egress proxy pod created by the Tigera Operator. This proxy pod that is also part of the Tailnet then forwards the packet out of the node via the NAT gateway but encapsulated on the Tailscale overlay network to the destination IPv4 tailnet-IP of the EC2 instance
