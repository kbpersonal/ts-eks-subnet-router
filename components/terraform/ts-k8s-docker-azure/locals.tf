...
locals {
  ...
  cluster_service_ipv4_cidr     = data.terraform_remote_state.azure_tfstate.outputs.cluster_service_ipv4_cidr
  ...
  aks_cluster_endpoint          = data.terraform_remote_state.azure_tfstate.outputs.aks_cluster_endpoint
  aks_cluster_ca_certificate    = base64decode(data.terraform_remote_state.azure_tfstate.outputs.aks_cluster_ca_certificate)
  aks_cluster_auth_token        = data.azurerm_kubernetes_cluster.credentials.kube_admin_config[0].token
